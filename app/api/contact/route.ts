...
export async function POST(request: NextRequest) {
  ...
  if (recent.length >= MAX_ATTEMPTS) {
    return NextResponse.json(
      { message: "Too many messages were submitted. Please wait and try again." },
      { status: 429 },
    );
  }
  ...
  if (now - parsed.data.startedAt < 2500) {
    return NextResponse.json(
  ...
  if (
    !(await verifyTurnstile(
      parsed.data.turnstileToken || parsed.data["cf-turnstile-response"],
      ip,
    ))
  ) {
    return NextResponse.json(
  ...
  const sender =
    ...
    "LomnickPro Website <onboarding@resend.dev>";
  ...
  try {
    ...
    console.error("contact_provider_rejected", {
  ...
  if (!response.ok) {
    console.error("contact_provider_rejected", {
      ...
      body: providerBody,
      senderDomain: sender.includes("@")
        ? sender.split("@").pop()?.replace(">", "")
        : "unknown",
    });
    return NextResponse.json(
      { message: "Your message could not be delivered. Please try again later." },
  ...
  console.info("contact_provider_accepted", {
    category: "accepted",
    status: response.status,
  });
